...
输出格式为JSON。`,
      templatePrompt: '请为章节内容生成{sceneCount}个分镜场景，总时长约{totalDuration}秒。',
      parameters: {
        temperature: 0.7,
        maxTokens: 2000,
      },
...
输出英文提示词，用逗号分隔。`,
      templatePrompt: '请为场景生成详细的AI绘图提示词。',
      parameters: {
        temperature: 0.8,
        maxTokens: 500,
      },
      version: 1,
      isActive: true,
      description: '文生图提示词优化配置',
    },
    {
      agentType: 'VIDEO_OPTIMIZER',
      name: '图生视频优化器 v1',
      systemPrompt: `你是一个专业的视频运动描述专家。你的任务是为静态图片生成合适的运动提示词。
...
输出简洁的英文运动描述。`,
  ...
  for (const config of agentConfigs) {
    const existing = await prisma.agentPromptConfig.findFirst({
      where: {
    ...
    if (!existing) {
      ...
      console.log(`✅ 创建Agent配置: ${config.name}`);
    } else {
      console.log(`⏭️  跳过已存在的配置: ${config.name}`);
    }
  }
